...
typedef int E; // Define the type of elements being used.
...
public:
    ...
    void initThread(Array2D<E> *m_matrix_C,Array2D<E> *m_matrix_M4, Array2D<E> *m_matrix_M5, Array2D<E> *m_matrix_M7, 
    	...
    	m_p_matrix_resultA = new Array2D<E>(size/2, size/2, 0, 0); // Buffer matrix to store additions, subtractions and copies
    	m_p_matrix_resultB = new Array2D<E>(size/2, size/2, 0, 0); // Buffer matrix to store additions, subtractions and copies
    	m_p_matrix_M = new Array2D<E>(size, size); 	         // Auxiliar matrix to store result C from A*B
    	m_p_matrix_A = new Array2D<E>(); 		   // Auxiliar matrix to store A
    	m_p_matrix_B = new Array2D<E>(); 		   // Auxiliar matrix to store B
              	...
              	for(aux_i = i; aux_i < aux_limit_i; ++aux_i) {
                	for(aux_j = j; aux_j < aux_limit_j; ++aux_j) {
                    	...
                    	for(aux_k = k; aux_k < unroll_limit; aux_k+=unroll_factor) {
                        	acc0 += A (endA + aux_k) * B (aux_k+ini_i, aux_j);
                        	acc1 += A (endA + aux_k+1) * B (aux_k+ini_i+1, aux_j);
                        	acc2 += A (endA + aux_k+2) * B (aux_k+ini_i+2, aux_j);
                        	acc3 += A (endA + aux_k+3) * B (aux_k+ini_i+3, aux_j);
                        	acc4 += A (endA + aux_k+4) * B (aux_k+ini_i+4, aux_j);
                        } 
                        ...
                        for (; aux_k < aux_limit_k; ++aux_k)
                        	C(endC) += A (endA + aux_k) * B (aux_k+ini_i, aux_j);    
...
void strassenMultiplication(const Array2D<E> &matrix_A, const Array2D<E> &matrix_B, int ini_i, int ini_j, int size,
    ...
    if (size <= g_truncateSize){ // Small enough to run standard multiplication
        ...
        // Matrix A quadrants
        Array2D<E> submatrix_A11(half_size, half_size);
        Array2D<E> submatrix_A12(half_size, half_size);
        Array2D<E> submatrix_A21(half_size, half_size);
        Array2D<E> submatrix_A22(half_size, half_size);
      
        // Matrix B quadrants
        Array2D<E> submatrix_B11(half_size, half_size);
        Array2D<E> submatrix_B12(half_size, half_size);
        Array2D<E> submatrix_B21(half_size, half_size);
        Array2D<E> submatrix_B22(half_size, half_size);
        ...
        Array2D<E> matrix_M1(0, 0);
        Array2D<E> matrix_M2(0, 0);
        matrix_M1.build(half_size, half_size, 0, 0);
        matrix_M2.build(half_size, half_size, 0, 0);
        
        // Getting submatrices from A and B
        for (int i = 0; i < half_size; ++i) {
 			for (int j = 0; j < half_size; ++j) {
 				submatrix_A11(i, j) = matrix_A(i+ini_i, j+ini_j);
 				submatrix_A12(i, j) = matrix_A(i+ini_i, j+ini_j+half_size);
 				submatrix_A21(i, j) = matrix_A(i+ini_i+half_size, j+ini_j); 
 				submatrix_A22(i, j) = matrix_A(i+ini_i+half_size, j+ini_j+half_size);

 				submatrix_B11(i, j) = matrix_B(i+ini_i, j+ini_j);
 				submatrix_B12(i, j) = matrix_B(i+ini_i, j+ini_j+half_size);
 				submatrix_B21(i, j) = matrix_B(i+ini_i+half_size, j+ini_j); 
 				submatrix_B22(i, j) = matrix_B(i+ini_i+half_size, j+ini_j+half_size);
 			}
		}
...
void parallelStrassenMultiplication(const Array2D<E> &matrix_A, const Array2D<E> &matrix_B, int numThreads, int size,
	...
	Array2D<E> matrix_M4(half_size, half_size); // Half_size is the size of a quadrant
	Array2D<E> matrix_M5(half_size, half_size);
	Array2D<E> matrix_M7(half_size, half_size);
	...
	Array2D<E> matrix_resultA(half_size, half_size);
	Array2D<E> matrix_resultB(half_size, half_size);
    ...
    for (int i = 0; i < half_size; ++i) {
		for (int j = 0; j < half_size; ++j) {
			...
			submatrix_B12(i, j) = matrix_B(i, j+half_size);
			submatrix_B21(i, j) = matrix_B(i+half_size, j); 
			submatrix_B22(i, j) = matrix_B(i+half_size, j+half_size);
		}
	}
...
void writeOutput(Array2D<E> &matrix_C, int dim1, int dim2) {
	ofstream file_out;
		...
		file_out.open("strassenMult_matrix_C.txt");
	#else
		file_out.open("strassenMultParallel_matrix_C.txt");	
	#endif
...
void writeReadTime(double time){
	ofstream file_out;
	#ifdef FORCE_SINGLE_CORE
		file_out.open("strassenMult_read_time.txt");
	#else
		file_out.open("strassenMultParallel_read_time.txt");
	#endif
	if (file_out.is_open()) {
		file_out << time;
		file_out.close();
...
void writeWriteTime(double time){
	ofstream file_out;
	#ifdef FORCE_SINGLE_CORE
		file_out.open("strassenMult_write_time.txt");
	#else
		file_out.open("strassenMultParallel_write_time.txt");
	#endif
	if (file_out.is_open()) {
		file_out << time;
		file_out.close();
...
void writeProcessTime(double time){
	ofstream file_out;
	#ifdef FORCE_SINGLE_CORE
		file_out.open("strassenMult_process_time.txt");
	#else
		file_out.open("strassenMultParallel_process_time.txt");
	#endif
	if (file_out.is_open()) {
		file_out << time;
		file_out.close();
...
int main() {	
    ...
    if ( size <= 2 ) {
		...
		if (single_core) {  
		    Array2D<E> matrix_resultA(size/2, size/2);   // Buffer for addition, subtraction and copy results
		    Array2D<E> matrix_resultB(size/2, size/2);   // Buffer for addition, subtraction and copy results
